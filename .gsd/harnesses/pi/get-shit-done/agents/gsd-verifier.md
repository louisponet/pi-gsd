...
tools: Read, Write, Bash, Grep, Glob, mcp__muninn__*
color: green
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
...
**Before verifying:** Recall known issues and past verification findings:
```
mcp__muninn__muninn_recall(context="<project> verification issues", vault="default")
```

**After verifying:** If you found significant issues or quality concerns, persist them:
```
...
pi-gsd-tools roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(pi-gsd-tools roadmap get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(pi-gsd-tools verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(pi-gsd-tools verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(pi-gsd-tools summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(pi-gsd-tools verify commits $COMMIT_HASHES)
fi

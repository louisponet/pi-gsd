...
tools: Read, Write, Edit, Bash, Grep, Glob, mcp__muninn__*
...
<muninndb_memory>
**MuninnDB** is the user's persistent memory system (MCP: `mcp__muninn__*`).

**On start:** Recall relevant context for the current task:
```
mcp__muninn__muninn_recall(context="<project> <phase> <task description>", vault="default")
```

**During execution:** If you discover a gotcha, make a non-obvious decision, or learn something
important about the codebase, store it:
```
mcp__muninn__muninn_remember(vault="default", concept="...", content="...", type="lesson", entities=[...])
```

**Decisions:** Use `mcp__muninn__muninn_decide` for significant technical decisions.

Keep memories atomic (one concept per call). Don't store transient plan state — that stays in `.planning/`.
</muninndb_memory>
...
**Project skills:** Check `.pi/skills/` or `.agents/skills/` directory if either exists:
...
**See @.pi/gsd/references/checkpoints.md**
...
```bash
pi-gsd-tools commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @.pi/gsd/templates/summary.md
...
pi-gsd-tools roadmap update-plan-progress "${PHASE_NUMBER}"
...
```bash
pi-gsd-tools state add-blocker "Blocker description"
```
</state_updates>

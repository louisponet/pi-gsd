...
tools: Read, Write, Bash, Glob, Grep, WebFetch, mcp__context7__*, mcp__muninn__*
color: green
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
...
**Before planning:** Recall past decisions and lessons for this project:
```
mcp__muninn__muninn_recall(context="<project> <phase scope>", vault="default")
```
This surfaces past architectural decisions, gotchas, and preferences that should inform the plan.

**After planning:** If you made significant design decisions or identified constraints, store them:
```
mcp__muninn__muninn_decide(decision="...", rationale="...", vault="default")
```
</muninndb_memory>
...
```bash
pi-gsd-tools commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
VALID=$(pi-gsd-tools frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(pi-gsd-tools verify plan-structure "$PLAN_PATH")
```
...
```bash
pi-gsd-tools commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>

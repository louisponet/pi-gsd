 ...
 *
 * Also injects portable path fields so workflows never need hardcoded absolute paths:
 *   gsd_bin         - relative path to gsd-tools.cjs from project root  (e.g. ".github/get-shit-done/bin/gsd-tools.cjs")
 *   gsd_harness_dir - relative path to the harness config dir            (e.g. ".github")
 *   gsd_root        - relative path to the get-shit-done install dir     (e.g. ".github/get-shit-done")
 * Workflows should use: node "$(echo $INIT | jq -r .gsd_bin)" <command>
 * or set GSD=$(echo $INIT | jq -r .gsd_bin) once at the top of each workflow.
 */
function withProjectRoot(cwd, result) {
    result.project_root = cwd;

    // Compute portable, CWD-relative paths to the GSD installation (#portability).
    // __dirname here is <configDir>/get-shit-done/bin/lib/ - resolve upward.
    const gsdBinAbs = path.join(__dirname, '..', 'gsd-tools.cjs');
    const gsdRootAbs = path.join(__dirname, '..', '..');
    const gsdHarnessDirAbs = path.join(__dirname, '..', '..', '..');
    result.gsd_bin = toPosixPath(path.relative(cwd, gsdBinAbs));
    result.gsd_root = toPosixPath(path.relative(cwd, gsdRootAbs));
    result.gsd_harness_dir = toPosixPath(path.relative(cwd, gsdHarnessDirAbs));
